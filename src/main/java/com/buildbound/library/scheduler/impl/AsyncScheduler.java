...
public class AsyncScheduler implements Scheduler {
    ...
    public @NotNull Task runTask(final @NotNull Runnable runnable) {
        return Task.bukkit(this.bukkitScheduler.runTaskAsynchronously(this.javaPlugin, runnable));
    }

    @Override
    public @NotNull Task runTaskLater(final @NotNull Runnable runnable, final @NotNull Duration duration) {
        return Task.bukkit(this.bukkitScheduler.runTaskLaterAsynchronously(
                this.javaPlugin,
                runnable,
    ...
    public @NotNull Task runTaskTimer(final @NotNull Runnable runnable, final @NotNull Duration duration) {
        return Task.bukkit(this.bukkitScheduler.runTaskTimerAsynchronously(
                this.javaPlugin,
                runnable,
                duration.to(TimeUnit.TICKS),
                duration.to(TimeUnit.TICKS)
